...
describe('doors', () => {
  it('opens on use, holds, then closes', () => {
    const door = createDoor(1, 1, false)
  ...
  it('locked doors refuse without the key and report the unlock', () => {
    const door = createDoor(1, 1, true)
    expect(operateDoor(door, false)).toBe('locked')
    expect(door.phase).toBe('closed')
    expect(operateDoor(door, true)).toBe('unlocked')
    expect(door.locked).toBe(false)
  })

  it('bounces back up when blocked while closing', () => {
    const door = createDoor(0, 0, false)
