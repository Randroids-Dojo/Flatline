...
Watched dependencies for Flatline. The agent runs the **Dependency Upgrade Gate** at two trigger points:
...
- **Why watched**: internally maintained, pre-1.0, breaking changes possible on every release. Flatline consumes the kit's portable + server modules.
