...
4. If main is broken, choose the hotfix first.
5. If a PR has unresolved actionable review comments, address them before new work.
6. Choose the highest-priority ready implementation task.
7. If no implementation task is ready, choose the highest-priority research task that can unblock one.
8. If no tasks are ready, create the missing research task from the GDD or report that the backlog is blocked.
...
7. Dots state is current.
8. PR review comments are handled.
9. CI is green.
10. The merge commit is deployed and smoked when a deploy target exists.
