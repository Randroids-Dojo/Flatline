...
- 2026-05-11: HUD redesigned into proper game-HUD regions. The previous layout (12 equal-sized pills in one horizontal row at the top-left) was reorganized into five named regions with a clear visual hierarchy: bottom-left **vitals** (240 px HP bar with green/amber/red zones, HP number overlay, dash chip below); bottom-right **armament** (weapon icon + name + ready state + big ammo readout); top-center **streak** panel (huge combo number with score subtitle and combo timer bar underneath); top-right **runstate** (wave chip with pulsing peak dot, run clock, kill count); top-left **status badges** (rage / score 2x / supply ready, only render when active). Pure-debug fields (enemy type / health, hits, billboard bucket) moved behind `practiceSettings.debugOverlays` so the production HUD is no longer a debug strip. Low-HP wobble is now scoped to the HP bar instead of every pill (a dying player feels the vitals shake, not the entire HUD swim). `pointer-events: none` on the HUD root lets clicks pass to the render canvas. New `hpZone(hp)` helper maps health to the green / amber / red gradient. All required test ids (`hud`, `weapon-ready`, `combo-pill` + `combo-timer-bar`, `dash-ready`, `wave-pill`) preserved so smoke + hud-motion specs still pass. Files: `src/components/FlatlineGame.tsx`, `app/globals.css`. PR #pending. Status stays `done` (all required HUD elements still present; this slice is a UX revision, not a feature add).
- 2026-05-09: Cartoon-title-card ink-bleed filter on HUD pill borders + status flip to `done`. Closes F-022 and the remaining partial of F-019. The HUD root mounts a hidden `<svg width="0" height="0">` carrying a `hud-ink-bleed` filter (`<feTurbulence type="fractalNoise" baseFrequency="0.9" numOctaves="2" seed="3"/>` + `<feDisplacementMap in="SourceGraphic" scale="2"/>`). `.hud-pill::before` paints a 1px cream outline aligned to the pill rect via `inset: -1px` and routes only that outline through the filter, so the pill text + icons stay crisp. `@media (prefers-reduced-motion: reduce)` clears the filter. `pointer-events: none` keeps the overlay click-through. With the rubber-hose icons (PR #130), the wobble + film-grain + ink-splatter overlays from PR #59, and the damage-direction indicator from PR #49 already landed, every cartoon-title-card spec checkbox plus every required HUD element is now visible. Files: `src/components/FlatlineGame.tsx`, `app/globals.css`. PR #131.
