...
**Case Board** (the manor tree): a 5x4 grid of nodes joined by string. Fog of war: a node is visible only when purchased or adjacent to a purchase, hidden nodes show face-down photos. Sixteen nodes (the pre-owned office root plus fifteen purchases): hp (two nodes, deeper one bigger), damage (two), speed, start armor, cheddar gain, max ammo, drop luck, fire rate, automap range, the Gun Locker (opens the Armory), The Fence (opens relics), and the Floor Safe plus Offshore Account (rent banking). Rank cost grows about 60% per rank; past 30 total ranks a labor-cost inflation adds 10 per rank purchased anywhere (the RL2 rule).
