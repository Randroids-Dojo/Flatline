...
- 2026-05-05: Snap stale `(0, 0)` joystick origins to the real finger position on the next `pointermove`. Real-device confirmation (Android Chrome on a Vercel preview, screenshot in conversation): even after the prior slice removed the `(0, 0)` rejection guards, the move stick visibly anchored at the top-left corner because the browser's first touch event reported `clientX === 0 && clientY === 0`. Added `rebaseOriginIfStale(joystick, x, y)` inside the touch effect; on every `pointermove`, if the joystick's origin is exactly `(0, 0)` and the incoming pointer coordinate is not, the origin (and the current point) snap to the new coordinate. The visual then jumps from the corner to the finger and the joystick reads as a normal small displacement instead of a saturated max-deflection. The fix is local to the move/look paths in `moveTouch` and is harmless on devices that never produce a `(0, 0)` first event. VibeRacer's `useTouchControls` was reviewed for prior art and uses the same `e.clientX/Y` read without any special handling. Files: `src/components/FlatlineGame.tsx`. PR #TBD.
