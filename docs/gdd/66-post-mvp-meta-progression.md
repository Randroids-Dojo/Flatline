...
- Cross-device sync is shipped through `/api/upgrade-wallet` (route in `app/api/upgrade-wallet/route.ts`, client wrapper in `src/lib/sharedUpgradeWallet.ts`). Per-device opaque uuid in `flatline.playerId.v1` (`src/lib/playerId.ts`). Server record is the highest-water-mark merge of every push; a stale or tampered upload can never regress credits, lifetime earnings, or any tier.
...
- 2026-05-10: Slice C shipped. Cross-device sync via Upstash. New `app/api/upgrade-wallet/route.ts` (GET / POST), client wrapper `src/lib/sharedUpgradeWallet.ts` (zod schema, highest-water-mark `mergeWallets`, KV ops via vibekit's `getKv`), and per-device opaque uuid in `src/lib/playerId.ts` (`flatline.playerId.v1`). KV key shape `flatline:wallet:{playerId}`; rate limit `flatline:ratelimit:wallet:ip:{ip}` at 30 syncs / 60s per IP. `src/components/FlatlineGame.tsx` adds a first-mount effect that ensures a player id, GETs the server record, merges with local, persists, and pushes back; `purchaseUpgrade` and `finishRun` now also fire-and-forget the push. Errors swallowed so offline play is unchanged. New tests: `src/lib/playerId.test.ts`, `src/lib/sharedUpgradeWallet.test.ts`, `tests/api.upgrade-wallet.test.ts`. PR #pending.
- 2026-05-10: Slice B shipped. Tree extended with `startingAmmo`, `weaponDamage`, and `moveSpeed`, each at 5 tiers on the same geometric `5/10/20/40/80` cost ramp. Per-tier effects: `+1` to both Boomstick and Inkblaster max ammo; `+10%` weapon damage; `+4%` move speed. New helpers in `src/game/upgradeTree.ts`: `effectiveMaxAmmoBonus`, `effectiveDamageMultiplier`, `effectiveMoveSpeedMultiplier`, plus `UPGRADE_STAT_IDS` so the UI can data-drive its row list. `src/game/weapons.ts` extended `createWeaponAmmo` and `collectWeaponAmmo` with optional `maxAmmoBonus` arg (default 0, preserves all existing callers and tests). `src/components/FlatlineGame.tsx` threads the bonus into run-start ammo init, the supply pickup `wantsSupply` check, the supply pickup refill, both weapon damage application sites (hitscan and inkblaster splash), and the per-frame movement multiplier. `UpgradePanel` refactored into a config-driven row list; CSS already covers the row layout. `src/lib/upgradeWallet.ts` schema extended with the three new fields, with a one-way migrator that fills missing fields with 0 so existing v1 wallets (Max HP only) keep their tier on first read. Status flips `partial` to `done`. PR #152.
