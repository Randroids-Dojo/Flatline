...
- 2026-05-09: Leaderboard rows now display all four GDD-spec columns: score, time, kills, accuracy. `LocalLeaderboard` and the inline `SharedLeaderboardPanel` rendering in `src/components/FlatlineGame.tsx` add `<span class="leaderboard-time/kills/accuracy">` cells per entry; `formatAccuracyPercent(ratio)` is a new local helper alongside the existing `formatTime(ms)`. `app/globals.css` widens the leaderboard from `max-width: 360px` to `420px`, switches the row grid from `1fr auto` to `minmax(0, 1fr) auto auto auto auto` for the new columns, and adds tabular-nums + a 12px secondary-text style so the numeric columns line up. Status flips to `done` because every checkbox in the spec ships: close-range bonus, weapon-variety bonus, no-damage-streak bonus, accuracy bonus, and leaderboard columns. Files: `src/components/FlatlineGame.tsx`, `app/globals.css`. PR #123.
- 2026-05-04: Added four scoring bonuses to fill the GDD spec. `src/game/scoring.ts` exposes `CLOSE_RANGE_THRESHOLD`, `CLOSE_RANGE_BONUS`, `WEAPON_VARIETY_BONUS`, `NO_DAMAGE_STREAK_BONUS`, and `ACCURACY_BONUS_MULTIPLIER`. `recordKill` now takes a `RecordKillOptions` object (`distance`, `weapon`, `tookDamageSinceLastKill`) and applies +50 close-range, +75 first-kill-per-weapon, +30 no-damage-streak per kill. `accuracyBonus(score)` is folded into `finalScore` (500x accuracy). `src/components/FlatlineGame.tsx` adds a `tookDamageSinceLastKillRef` flag set on hazard ticks and enemy melee landings, threads enemy distance + selected weapon + the flag into `recordKill`, exposes `closeRangeKills` / `weaponsUsed` / `bestNoDamageStreak` on `RunSummary`, and renders three new run-summary rows. Tests in `src/game/scoring.test.ts`. PR #123. Status stays `partial` because leaderboard columns for the new tallies are still local-only.
