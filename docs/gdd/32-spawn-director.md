...
- 2026-05-17: Endless roguelike spawn escalation. The pressure curve now starts at 4, reaches 10 at 300 s, and keeps adding one pressure point per minute afterward. The active enemy cap grows from 8 to 10 to 12, then toward 16 to preserve render headroom. A separate active-enemy floor now forces the room toward 5 / 6 / 8 / 10 live enemies as the run ages. Runtime top-up directly creates all missing enemies while below the floor, bypassing pressure so a few high-pressure enemies cannot stall respawning. Files: `src/game/spawnDirector.ts`, `src/game/spawnDirector.test.ts`, `src/components/FlatlineGame.tsx`. PR pending.
