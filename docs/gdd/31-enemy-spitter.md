...
- 2026-05-09: Dedicated spitter sprite atlas (closes the placeholder gap noted in the 2026-05-06 v1 entry). New `drawSpitter` variant in `scripts/generate-enemy-variant-atlases.mjs` paints a tall lanky body in canonical olive (`colors.gray`) with two thin stalk legs, a teal projectile sac on the belly that pulses across the four idle frames, and a danger-red eye on a short stalk. Hurt frames lean back and tint to `bruteRed`; death collapses to a puddle with the sac splat. 8 angles x (4 idle + 2 hurt + 4 death) = 80 frames, packed into `public/assets/enemies/spitter/spitter.png` plus `spitter.atlas.json`. `enemyAtlasTypes` in `FlatlineGame.tsx` now includes `'spitter'` so the atlas loads at run start. `src/game/spriteAtlas.test.ts` validates the new atlas alongside grunt / skitter / brute. The runtime windup tint from `src/game/spitterCharge.ts` continues to apply on top of the new sprite. Status flips to `done`. Files: `scripts/generate-enemy-variant-atlases.mjs`, `public/assets/enemies/spitter/spitter.png`, `public/assets/enemies/spitter/spitter.atlas.json`, `src/components/FlatlineGame.tsx`, `src/game/spriteAtlas.test.ts`. PR #127.
